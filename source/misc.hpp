...
{
public:
    static bool lines_intersect(std::complex<double> L1_a, std::complex<double> L1_b, std::complex<double> L2_a, std::complex<double> L2_b)
    {
        ...
        if(L1_a.real() == L1_b.real()) l1_vert = true;
        if(L2_a.real() == L2_b.real()) l2_vert = true;
        if(L1_a.imag() == L1_b.imag()) l1_hor = true;
        if(L2_a.imag() == L2_b.imag()) l2_hor = true;

        // check if in line range

        bool l1_x_in_range = (l1_vert && fabs(X_intersect - L1_a.real()) < 1e-8) ||
                             (X_intersect >= std::min(L1_a.real(), L1_b.real()) && X_intersect <= std::max(L1_a.real(), L1_b.real()));

        bool l2_x_in_range = (l2_vert && fabs(X_intersect - L2_a.real()) < 1e-8) ||
                             (X_intersect >= std::min(L2_a.real(), L2_b.real()) && X_intersect <= std::max(L2_a.real(), L2_b.real()));

        bool l1_y_in_range = (l1_hor && fabs(Y_intersect - L1_a.imag()) < 1e-8) ||
                             (Y_intersect >= std::min(L1_a.imag(), L1_b.imag()) && Y_intersect <= std::max(L1_a.imag(), L1_b.imag()));

        bool l2_y_in_range = (l2_hor && fabs(Y_intersect - L2_a.imag()) < 1e-8) ||
                             (Y_intersect >= std::min(L2_a.imag(), L2_b.imag()) && Y_intersect <= std::max(L2_a.imag(), L2_b.imag()));
